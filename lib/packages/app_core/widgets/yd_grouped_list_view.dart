...
///
@immutable
class YDGroupedListView<T, E> extends StatefulWidget {
  ...
  final Widget Function(BuildContext context, T element, List elements)?
      itemBuilder;

  /// Called to build children for the list with
  /// 0 <= element, index < elements.length
  final Widget Function(
          BuildContext context, T element, int index, List elements)?
      indexedItemBuilder;
  ...
  /// Creates a [YDGroupedListView]
  const YDGroupedListView({
    super.key,
  ...
  @override
  State<StatefulWidget> createState() => _YDGroupedListViewState<T, E>();
}

class _YDGroupedListViewState<T, E> extends State<YDGroupedListView<T, E>> {
  ...
  Widget _buildItem(context, int index) {
    ...
    return KeyedSubtree(
      ...
      child: widget.indexedItemBuilder != null
          ? widget.indexedItemBuilder!(context, value, index, _sortedElements)
          : widget.itemBuilder!(context, value, _sortedElements),
    );
  }
